...
function add_to_cart(info){
  ...
  var cart_retrieved = localStorage.getItem("cart") || null
...
function display_notif(type, info="No info provided"){
  ...
  switch (type){
    case "add to cart":
      toastr.success(info.item_description + ': BDT' + info.price, 'Added to Cart');
      break;
...
function load_cart(){
  ...
  if (cart !== null && cart.length > 0){
    for (var i = 0; i < cart.length; i++) {
      ...
      item_price.innerHTML = "BDT"+cart[i].price;
